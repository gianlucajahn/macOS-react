...
const inputWork = async function (query: string) {
    ...
    const currentResponse = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${query}&appid=${apiKey}&units=metric`);
    ...
    forecast.list.forEach((interval: intervalType, i: number) => {
      const newIntervalObject = {
        ...
        hours: new Date(interval.dt * 1000).getHours() > 12 ? new Date(interval.dt * 1000).getHours() - 12 + "PM" :  new Date(interval.dt * 1000).getHours() + "AM",
